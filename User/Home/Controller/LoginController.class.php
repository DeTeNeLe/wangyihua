...
class LoginController extends Controller {
	...
	public function sendPhone() 
	{
		...
		}else
		{
		   //sendCheckCode($_SESSION['check_account'],$_SESSION['check_phone'],2);
            sendAliCheckCode($_SESSION['check_account'],$_SESSION['check_phone'],2);
    ...
    public function logincl() 
	{
        ...
        if (IS_POST) 
		{
            ...
            if (!$this->check_verify($verCode)) {
                $this->ajaxReturn( array('nr'=>'验证码不正确，请刷新验证码！','sf'=>0) );
            }
    ...
    public function cspaycl($data) {
        ...
        if($time-$reg_time > $reg_feng_days*3600*24){
            ...
            if($tgbz == 0){
                ...
                $this->error('由于你注册之后未在'.$reg_feng_days.'天内买入,我们无法为你继续提供服务!');
            }
        }*/
        $ppdd = M('ppdd');
        $where = array();
    ...
    public function edit_pass()
    {
        ...
        $pwd_second_two = I('post.pwd_second_two');
        if( empty($second_pwd) )
        {
            $this->error('请填写二级密码!');
        }
        ...
        if( $pwd != $pwd_two ){
            $this->error('一级两次密码不一致!');
        }
        if( $second_pwd != $pwd_second_two ){
            $this->error('二级密码两次密码不一致');
        }


        $where['UE_account'] = $username;
        $where['UE_phone'] = $mobile;
        $update['UE_password'] = md5($pwd);
        $update['UE_secpwd'] = md5($second_pwd);//二级密码
    ...
    public function sendCodeToPhone()
    {
        $mobile = I('post.mobile');
        $username = I('post.username');
        //$cur_user = M('user')->where("UE_account = '" . $mobile . "' or UE_phone = '" .$mobile ."'")->find();
        $cur_user = M('user')->where("UE_phone = '" .$mobile ."'")->find();

        if( empty($cur_user) )
        {
            return_die_ajax('手机号码不存在!',true,0);
        }

        if( !empty($cur_user) && $username != $cur_user['ue_account'] ){
            return_die_ajax('账号不存在!',true,0);
        }
        ...
        }else
        {
            ...
            sendAliCheckCode($cur_user['ue_account'],$cur_user['ue_phone'],1);
    ...
    function test()
    {
        //$res = sendSMS('13119710425','你好，这里是测试短信的接口【富怡资本】','','http://api.sms.cn/sms/',0);
        //dump($res);
    }
}
