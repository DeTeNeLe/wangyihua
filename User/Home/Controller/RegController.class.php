...
class RegController extends Controller {
    ...
    public function sendPhone() 
    {
        ...
        }else
        {
           ...
           sendAliCheckCode(I('post.username'),I('post.mobile'),1);
    ...
    public function regadd() {
        ...
        } else 
        {
            ...
            $data_arr["levelname"] = 'v1'; // 用户初始为v1等级
            ...
            $data_arr["yhzh"] = !empty($data_P['bankaccount']) ? authcode($data_P['bankaccount'],'ENCODE',C('base64_code_pwd'),0):'';
            $data_arr["yhmc"] = !empty($data_P['bankname']) ? authcode($data_P['bankname'],'ENCODE',C('base64_code_pwd'),0) : '';
            $data_arr["yhckr"] = !empty($data_P['bankowner']) ? authcode($data_P['bankowner'],'ENCODE',C('base64_code_pwd'),0):'';
            $data_arr["yhzhxx"] = !empty($data_P['bankaddr']) ? authcode($data_P['bankaddr'],'ENCODE',C('base64_code_pwd'),0):'';
            $data_arr["zfb"] = !empty($data_P['alipay']) ? authcode($data_P['alipay'],'ENCODE',C('base64_code_pwd'),0):'';
            $data_arr["weixin"] = !empty($data_P['weixin']) ? authcode($data_P['weixin'],'ENCODE',C('base64_code_pwd'),0):'';
            $data_arr["remark"] = $data_P['bankmark'];
            $data_arr['tx_leiji'] = 0;
            $data_arr['fangzhuang'] = 1;//此处关闭防撞功能0，字段默认是开启1
            ...
            if ($data->create($data_arr)) {
                if (I('post.ty') <> 'ye') {
                    ...
                    if ($data->add()) {
                        ...
                        //新用户注册奖励【激活码】
                        //newuserjhm($data_P['username'], C('new_user_jhm_pass_card'), '新用户注册奖励通证积分（激活码）' . C('new_user_jhm_pass_card') . '个');
                        //新用户注册奖励通证积分【排单码】
                        newuserpdm($data_P['username'], C('new_user_jhm_pass_card'), '新用户注册奖励通证积分（排单码）' . C('new_user_jhm_pass_card') . '个');
                        return_die_ajax('注册成功',true,1);
                        //return_die_ajax($data_P['username'],true,1);//注冊成功
                    } else {
