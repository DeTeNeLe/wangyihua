...
class InfoController extends CommonController
{
    ...
    public function xgzlcl()
    {
        if (IS_POST) {
            ...
            //if ($tgbztj > 0) {
            //    $this->ajaxReturn(array('nr' => '买入成功后不可修改个人信息！', 'sf' => 0));
            //} else {
                $userxx = M('user')->where(array('UE_account' => $_SESSION['uname']))->find();
                $data_up['UE_phone'] = $data_P['new_phone_check'];
                $userxx_new = M('user')->where(array('UE_phone'=>$data_up['UE_phone']))->find();
                if( !empty($userxx_new) && $_SESSION['uname'] != $userxx_new['ue_account'] ){
                    $this->ajaxReturn(array('nr' => '手机号码，已存在！', 'sf' => 0));
                }
                $data_up['weixin'] = !empty($data_P['wechat']) ? authcode($data_P['wechat'],'ENCODE',C('base64_code_pwd'),0):'';
                $data_up['zfb'] = !empty($data_P['alipay']) ? authcode($data_P['alipay'],'ENCODE',C('base64_code_pwd'),0):'';
                $data_up['yhmc'] = !empty($data_P['yhmc']) ? authcode($data_P['yhmc'],'ENCODE',C('base64_code_pwd'),0):'';
                $data_up['remark'] = $data_P['remark'];
                $data_up['yzf'] = !empty($data_P['yzf']) ? authcode($data_P['yzf'],'ENCODE',C('base64_code_pwd'),0):'';
                $data_up['yhzh'] = !empty($data_P['yhzh']) ? authcode($data_P['yhzh'],'ENCODE',C('base64_code_pwd'),0):'';
                $data_up['yhckr'] = !empty($data_P['yhckr']) ? authcode($data_P['yhckr'],'ENCODE',C('base64_code_pwd'),0):'';
                $data_up['yhzhxx'] = !empty($data_P['yhzhxx']) ? authcode($data_P['yhzhxx'],'ENCODE',C('base64_code_pwd'),0):'';
                $data_up['UE_theme'] = !empty($data_P['ue_theme']) ? $data_P['ue_theme']:'';
    ...
    public function xgyjmmclsec()
    {
        if (IS_POST) {
            ...
            if (!preg_match('/^[a-zA-Z0-9]{1,15}$/', $data_P ['xmm']))
            {
                ...
                $addaccount = M('user')->where(array(UE_account => $_SESSION ['uname']))->find();
                if ($addaccount['ue_secpwd'] <> md5($data_P['ymm'])) {
                    $this->ajaxReturn(array('nr' => '原密碼不正確', 'sf' => 0));
                }else {
                    $reg = M('user')->where(array(
                        'UE_ID' => $_SESSION ['uid']
                    ))->save(array('UE_secpwd' => md5($data_P['xmm'])));
                    if ($reg) {
                        $this->ajaxReturn(array('nr' => '修改成功!', 'sf' => 1));
                    } else {
                        $this->ajaxReturn(array('nr' => '修改失敗!', 'sf' => 0));
...
//jerry
                ...
                if ($count2 >= 60) {
                    ...
                    */
                    ...
                    if( $type == 0 ){
                        ...
                        $this->success("正在跳转" . $lx_he,'/Index/fill_in_order/',2);
                    }
    ...
    public function jhm()
    {
        ...
        //所有的总金额【已得到红利的总金额】
        $userget_UG_money = M('userget')->where(array('UG_dataType'=>'tgbz','UG_account'=>$_SESSION['uname']))->sum('UG_allGet');
        $userget_UG_money = !empty($userget_UG_money) ? $userget_UG_money : 0;

        //通过userget.varid与tgbz表中id对应，找出tgbz表中的mainid,然后计算出本金【总金额】
//        if( $userget_UG_money > 0 ) {
//            $all_tgbz_id_arr = M('userget')->field('varid')->where(array('UG_dataType' => 'tgbz', 'UG_account' => $_SESSION['uname'],'_string'=>'varid > 0'))->select();
//            $all_tgbz_id_arr_data = [];
//            foreach ($all_tgbz_id_arr as $item) {
//                $all_tgbz_id_arr_data[] = $item['varid'];
//            }
//
//            if (!empty($all_tgbz_id_arr_data)) {
//                $map['in'] = $all_tgbz_id_arr_data;
//                $map['user'] = array('eq', $_SESSION['uname']);
//                $all_tgbz_id_mainid_data = M('tgbz')->field('mainid')->where($map)->select();
//                $all_tgbz_id_mainid_arr = [];
//                foreach ($all_tgbz_id_mainid_data as $v) {
//                    $all_tgbz_id_mainid_arr[] = $v['mainid'];
//                }
//                $map2['in'] = $all_tgbz_id_mainid_arr;
//                $map2['user'] = array('eq', $_SESSION['uname']);
//                $map2['_string'] = "mainid = id";
//                $all_done_total_jb = M('tgbz')->field('mainid')->where($map2)->sum('total');
//                $all_done_total_jb = !empty($all_done_total_jb) ? $all_done_total_jb : 0;
//                $all_done_total_lx = $userget_UG_money - $all_done_total_jb;
//            }
//        }
        //未提取的利息
        $tgbz = M("tgbz");
        ...
        $result = $tgbz->where($map_tg)->select();
        $lx = 0;
...
//        }
        ...
        $list1 = $user_jj->where($map_jj)->select();
        //已提取的利息
        $ytq_lx = 0;
        foreach($list1 as $value){
            $ytq_lx += user_jj_zong_lx($value['id']);
        }
        $all_done_total_lx = $lx + $ytq_lx;//所有的利息总和【15%的部分】





        $this->assign('all_done_total_lx',$all_done_total_lx);//总的分割红利
   ...
   public function sendPhone_info()
   {
       ...
       {
           sendAliCheckCode($_SESSION['uname'],$phone,5);
           $this->ajaxReturn(array('nr' => '获取成功', 'sf' => 1));
       }
       $this->ajaxReturn(array('nr' => '已超过5次限制', 'sf' => 1));
    }
    ...
    {
        $accuserData = M('user')->field('ue_theme,ue_account,ue_phone,weixin')->where(array('UE_account'=>$this->userData['ue_accname']))->find();
        $this->assign('accuserData',$accuserData);
        $this->display();
    }
    ...
    public function trading_record()
    {
        ...
        $count = $tgbzModel->where($map1)->group('mainid')->count();
        $p = getpage($count, 10);

        $listOne = $tgbzModel->where($map1)->order('date DESC')->group('mainid')->limit($p->firstRow, $p->listRows)->select();
        $map['_string'] = 'mainid = id';
        $map['user'] = $_SESSION['uname'];
        $map['isprepay'] = 0;

        $list = [];
        foreach($listOne as $key => $item)
        {
            $total_data = $tgbzModel->where(array('mainid'=>$item['mainid']))->select();
            $item['cc_total'] = 0;//订单持仓总量
            $item['cc_total_num'] = 0;//订单持仓的总数量
            $item['kc'] = 0;//订单开仓积分
            $item['kc_num'] = 0;//订单开仓的订单数量
            $item['pc'] = 0;//订单平仓积分
            $item['pc_num'] = 0;//订单平仓的数量
            $item['kc_zt'] = 0;//开仓订单的状态  0未支付  1已支付 2部分支付
            $cur_kc_zt_num = 0;//开仓订单支付完成的数量【当前主id即mainid对应的数量】
            $item['pc_zt'] = 0;//平仓订单的状态  0未支付  1已支付 2部分支付
            $cur_pc_zt_num = 0;//平仓订单支付完成的数量【当前主id即mainid对应的数量】
            ...
            //开仓的状态
            if( $cur_kc_zt_num == 0 ){//开仓订单的状态  0未支付  1已支付 2部分支付
                $item['kc_zt'] = 0;//未支付
            }else if( $cur_kc_zt_num > 0 && $cur_kc_zt_num < $item['kc_num'] ){
                $item['kc_zt'] = 2;//部分完成
            }else if( $cur_kc_zt_num > 0 && $cur_kc_zt_num == $item['kc_num']){
                $item['kc_zt'] = 1;//已完成
            }
            ...
            //平仓的状态
            if( $cur_pc_zt_num == 0 ){//平仓订单的状态  0未支付  1已支付 2部分支付
                $item['pc_zt'] = 0;//未完成支付
            }else if( $cur_pc_zt_num > 0 && $cur_pc_zt_num < $item['pc_num'] ){
                $item['pc_zt'] = 2;//部分完成
            }else{
                $item['pc_zt'] = 1;//已完成
            }
        ...
        //w_peidui_lx();//未配对利息
        //w_peidui_day();//未配对天数
        //get_p_status();//获取当前订单的状态
        //get_tgbz_totaljb();//获取当前订单的总金额
        //get_tgbz_totaljb();
        //get_pro_num();//获取订单的拆分数量
        ...
        */
        ...
        $map['user'] = $_SESSION['uname'];
        $map['isprepay'] = 0;
        $listTwo = M('tgbz')->where($map)->select();
        */
