	...
	<div role="tabpanel" class="tab-pane active" style="overflow: auto">
		<div class="maincon">
			<div class="well bs-component">
				<legend style="text-align: left;font-weight: bold; border: none; color: white;">当前<?php echo C('pdm_name');?>数量：<?php echo ($userData['pdmnum']); ?></legend>
				<form class="form-horizontal">
					<fieldset>
						<legend style="border: none; color: white;"><?php echo C('pdm_name');?>转让 </legend>
						<span style="border: none; color: white;" class="help-block">转让无需手续费</span>
						<div class="form-group">
							<div>
								<input style="width: 80%; margin: 0px auto;" type="text" class="form-control" id="num_zz_user" placeholder="请输入对方账号或手机号">
							</div>
							<label for="num_zz_user" class="col-lg-4 col-xs-4 text-left control-label"></label>
						</div>
						<div class="form-group">
							<div>
								<input style="width: 80%; margin: 0px auto;" type="text" class="form-control" id="num_zz_nums" placeholder="请输入数量">
							</div>
							<label for="num_zz_nums" class="col-lg-4 col-xs-4 text-left control-label"></label>
						</div>
						<div class="form-group">
							<div>
								<input style="width: 80%; margin: 0px auto;" type="text" class="form-control" id="secpwd" placeholder="请输入二级密码">
							</div>
							<label for="secpwd" class="col-lg-4 col-xs-4 text-left control-label"></label>
						</div>
						<input style="height: 42px;width: 80%; margin: 0px auto;line-height: 28px; font-size: 18px;" type="button" class="form-control" value="提交" disabled="disabled" id="num_zz_submit">
					</fieldset>
				</form>
			</div>
			<div class="walletpanel">
				<div class="contitle active">
					<?php echo C('pdm_name');?> 消费纪录
				</div>
				<!--<table class="table">
					<tbody>
						<tr>
						...
						<?php if(is_array($paidan_list)): foreach($paidan_list as $key=>$v): ?><tr>
								<td><?php echo ($v["id"]); ?></td>
								<td><?php echo ($v["date"]); ?></td>
								<td>
									<?php if($v["type"] == 'zc'): ?>转出<?php endif; ?>
									<?php if($v["type"] == 'pd'): ?>排单<?php endif; ?>
				...
				<div>
					<?php if(is_array($paidan_list)): foreach($paidan_list as $key=>$v): ?><div class="order_bar">
							<h5 style="overflow: hidden;">
								<span style="float: left;">编号：<?php echo ($v["id"]); ?></span>
								<span style="float: right;">类型:
									<?php if($v["type"] == 'zc'): ?>转出<?php endif; ?>
									<?php if($v["type"] == 'pd'): ?>开仓<?php endif; ?>
									<?php if($v["type"] == 'zr'): ?>转入<?php endif; ?>
									<?php if($v["type"] == 'cz'): ?>充值<?php endif; ?>
									<?php if($v["type"] == 'yypd'): ?>预约<?php endif; ?>
									<?php if($v["type"] == 'zcjl'): ?>注册奖励<?php endif; ?>
									<?php if($v["type"] == 'wwcfkfh'): ?>未付款封号<?php endif; ?>
								</span>
							</h5>
							<p style="display: block; text-align: left">数量：<?php echo ($v["num"]); ?></p>
							<p style="display: block; text-align: left;">余额：<?php echo ($v["yue"]); ?></p>
							<p style="display: block; text-align: left;">时间：<?php echo ($v["date"]); ?></p>
							<p style="display: block; text-align: left">说明：<?php echo ($v["info"]); ?></p>
						</div><?php endforeach; endif; ?>
				</div>
				<!--<div class="fixed-table-pagination" style="display: block;">
					<div class="pull-left pagination-detail">
						<span class="pagination-info"> 共 <?php echo ($paidan_count); ?> 条纪录</span>
...
<script>
	...
	(function() {
		document.title = "<?php echo C('pdm_name');?>";
		//积分转账
		$(document).on('blur', '#num_zz_user', function() {
			...
			if(o <= 0) {
				...
				$('[for=\'num_zz_nums\']').text('需扣除0%手续费：' + (o * 0.0) + "<?php echo C('pdm_name');?>");
			...
			if(o > 0) {
				$.ajax({
					type: 'post',
					url: '/Home/Index/home_post',
					data: "act=get_userinfo&field=pdmnum&u_p=<?php echo ($userData['ue_account']); ?>",
					success: function(data) {
						if(data.sf == 1) {
							if(data.nr < parseInt(o)) {
								$('[for=\'num_zz_nums\']').html('<span style=\'color:#090\'>' + "您的<?php echo C('pdm_name');?>不足！" + '</span>');
								$('#num_zz_submit').removeAttr('disabled');
							}
			...
			layer.confirm('是否向会员' + to_user + '转帐' + num + "<?php echo C('pdm_name');?>？", {
					title: '请确认',
				},
				function(index, layero) {
					$.ajax({
						type: 'post',
						url: '/Home/Index/home_post',
						data: 'act=paidan_zc&to_user=' + to_user + '&num=' + num + '&secpwd=' + secpwd,
						success: function(data) {
							if(data.sf == 1) {
								layer.closeAll();
								layer.msg('转帐成功');
