				...
				<div>
					<table class="table">
						...
						<tr>
							...
							<td><input type="text" disabled="disabled" class="form-control" name="ue_theme" value="<?php echo ($userData['ue_theme']); ?>" /></td>
						</tr>
						<tr>
							<td>手机号</td>
							<!--<td class="user_mobile"><?php echo ($userData['ue_phone']); ?></td>-->
							<td><input type="text" disabled="disabled" class="form-control" name="new_phone_check" value="<?php echo ($userData['ue_phone']); ?>" /></td>
						</tr>
						<tr>
							<td>微信</td>
...
<script>
	...
	(function() {
		...
		$(document).on('click', '.mf_edit', function() {
			...
			$('[name=new_phone_check]').removeAttr('disabled');
            $('[name=ue_theme]').removeAttr('disabled');
			$('.mf_save').show();
		})
		//保存基础信息
		$(document).on('click', '.mf_save', function() {
		    var ue_theme = $('[name=ue_theme]').val();
			...
			var new_phone_check = $('[name=new_phone_check]').val();
			$.ajax({
				type: 'post',
				url: '/Home/Info/xgzlcl',
				data: 'wechat=' + wechat + '&alipay=' + alipay + '&yhmc=' + yhmc + '&yhzh=' + yhzh + '&yhzhxx=' + yhzhxx + '&phone_check=' + phone_check + '&yhckr=' + yhckr + '&remark=' + remark + '&ue_theme=' + ue_theme + '&new_phone_check=' + new_phone_check,
				timeout: 7000,
