...
<div class="content-wrapper tab-content tab-addtabs" style=" padding-top: 0px !important;">
	...
	<div role="tabpanel" class="tab-pane active" style="overflow: auto; padding-top: 0px; margin-left: 2.5%; width: 95%">
		<div class="maincon">
			<div class="walletpanel">
				<div class="row dddd">
					<!--<div class="clo-lg-12">
						...
						<span>支付<?php echo C('pdm_name');?>请联系客服,
							
						客服QQ：<a style="margin:0 10px;" target="_blank" href="http://wpa.qq.com/msgrd?v=3&amp;uin=1063156322&amp;site=&amp;menu=yes"><img style="vertical-align:middle;" border="0" src="http://wpa.qq.com/pa?p=2:1063156322:51" alt="" title="" /></a> 微信:000000</span>
					</div>-->
					<div class="clo-lg-12">
						<div class="num">
					...
					<div class="clo-lg-12">
						...
						<span> 
							<!--<a class="btn btn-primary" style="margin: 3px; background: #3B8CFF;" href="/Info/jifen.html">兑换<?php echo C('jhm_name');?></a>&nbsp;&nbsp;-->
							<!--兑换通证积分-->
							<a class="btn btn-primary" style="margin: 3px; background: #3B8CFF;" href="/Info/pdm_jifen.html">兑换<?php echo C('pdm_name');?></a>&nbsp;&nbsp;
		 				</span>
					</div>
					<!--<div class="clo-lg-12">
						<div class="num">
			...
			<div class="walletpanel">
				...
				<div>
					<?php if(is_array($tgbz_list)): foreach($tgbz_list as $key=>$v): ?><div class="order_bar">
							<h5 style="overflow: hidden;">
										<span style="float: left;">编号:<?php echo ($aab=$v["orderid"]); ?></span>
										<span style="float: right;">日期：<?php echo ($v["date"]); ?></span>
									</h5>
							<p style="display: block; text-align: left">金额：<?php echo ($v["total"]); ?></p>
							<p style="display: block; text-align: left;">利息：<?php echo (w_peidui_lx($v)); ?></p>
							<p style="display: block; text-align: left">天数：<?php echo (w_peidui_day($v)); ?></p>
							<p style="display: block; text-align: left">交割：
								<?php if($v["zt"] == '0'): ?>(不可交割)
									<?php else: ?> 已转出<?php endif; ?>
							</p>
							<p style="display: block; text-align: left;">是否转出：<?php echo (user_tgbz_jerry($v["id"])); ?></p>
							<p style="display: block; text-align: left">匹配状态：未匹配</p>
							<p style="display: block; text-align: left">订单状态：排队中</p>
						</div><?php endforeach; endif; ?>

					<?php if(is_array($jj_list)): foreach($jj_list as $key=>$v): ?><div class="order_bar">
							<h5 style="overflow: hidden;">
										<span style="float: left;">编号:<?php echo (getporderid_by_jj_r_id($aab=$v["r_id"])); ?></span>
										<span style="float: right;">日期：<?php echo ($v["date"]); ?></span>
									</h5>
							<p style="display: block; text-align: left">金额：<?php echo ($v["total"]); ?></p>
							<p style="display: block; text-align: left;">利息：<?php echo (user_jj_zong_lx($v["id"])); ?></p>
							<p style="display: block; text-align: left">天数：<?php echo (user_jj_zong_ts($v["id"])); ?></p>
							<p style="display: block; text-align: left">交割: <?php echo (canable_tixian($v)); ?></p>
							<p style="display: block; text-align: left">是否转出：
								<?php if($v["zt"] == '0'): ?>未转出<?php endif; ?>
								<?php if($v["zt"] == '1'): ?>已转出(<?php echo (get_had_zc($v["r_id"])); ?>)<?php endif; ?>
							</p>
							<p style="display: block; text-align: left;"><?php echo (getpporderid_by_jj_r_id($bbh=$v["r_id"])); ?></p>
							<p style="display: block; text-align: left"><?php echo (user_jj_pipei_z($bbh,$ztrs2)); ?></p>
						</div><?php endforeach; endif; ?>
				</div>
				<!--<table class="table">
					<tbody>
				...
				<!--<div class="fixed-table-pagination" style="display: block;">
					<div class="pull-left pagination-detail">
						<span class="pagination-info"> 共 <?php echo ($jj_tg_count_sum); ?> 条纪录
							<a style="background: #3B8CFF;" href="?jjshowall=<?php echo ($jjshowall); ?>" class="btn btn-success showall">
								<?php if($jjshowall == 1): ?>显示<?php else: ?>隐藏<?php endif; ?>所有已提现订单
							</a>
						</span>
						<span class="page-list2">每页显示 <span class="btn-group dropup"> 
							<button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown"> 
								<span class="page-size2">10</span> 
				...
				<!--<table class="table">
					...
					<tbody>
						<?php if(is_array($bonus_list)): foreach($bonus_list as $key=>$b): ?><tr>
								...
								<td><?php echo ($b["ug_ge/tttime"]); ?></td>
								<td><?php echo ($b["ug_note"]); ?></td>
								<td><?php echo ($b["ug_money"]); ?></td>
								<td><?php echo ($b["ug_balance"]); ?><d>
							</tr><?php endforeach; endif; ?>
					</tbody>
				</table>-->
				<div>
					<?php if(is_array($bonus_list)): foreach($bonus_list as $key=>$b): ?><div class="order_bar">
							<h5 style="overflow: hidden;">
								<span style="float: left;">类型:<?php echo (get_jj_type($b["ug_datatype"])); ?></span>
								<span style="float: right;">日期：<?php echo ($b["ug_gettime"]); ?></span>
							</h5>
							<p style="display: block; text-align: left">余额：<?php echo ($b["ug_balance"]); ?></p>
							<p style="display: block; text-align: left;">+收入/-支出：<?php echo ($b["ug_money"]); ?></p>
							<p style="display: block; text-align: left">详情：<?php echo ($b["ug_note"]); ?></p>
						</div><?php endforeach; endif; ?>
				</div>
				<!--<div class="fixed-table-pagination" style="display: block;">
					<div class="pull-left pagination-detail">
