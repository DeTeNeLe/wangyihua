		...
		<div class="maincon">
			<div class="orderlist">
				<?php if(is_array($pp_p_list)): foreach($pp_p_list as $key=>$pp_p): ?><div class="l_item lout status_pp_p_<?php echo (get_pp_p_status($pp_p["pporderid"])); ?>" act="viewinfo" data="<?php echo ($pp_p["pporderid"]); ?>" style="background: #002639; color: white;">
						...
						<div class=" lout status_pp_p_<?php echo (get_pp_p_status($pp_p["pporderid"])); ?>" act="viewinfo" data="<?php echo ($pp_p["pporderid"]); ?>" style="margin-left:0px;">
							...
							<span>
								<!--(匹配卖出订单<?php echo ($pp_p['jsbz_data']['orderid']); ?>)-->
								已匹配开仓订单
							</span>
							<div>
								<!--<div class="bj status_pp_p_<?php echo (get_pp_p_status($pp_p["pporderid"])); ?>"></div>-->
								<div class="bj">
									<span>需要打款金额：</span><span><?php echo ($pp_p["jb"]); ?>元</span>
				...
				<?php if(is_array($pp_g_list)): foreach($pp_g_list as $key=>$pp_g): ?><div class="l_item lout status_pp_p_<?php echo (get_pp_g_status($pp_g["pporderid"])); ?>" act="viewinfo" data="<?php echo ($pp_g["pporderid"]); ?>" style="background: #002639; color: white;">
						<span style="color: white; display: block; font-size: 18px;line-height: 44px; padding: 0px 10px;  border-bottom: 1px solid #eee;" class="orderid">
							<?php echo ($pp_g["pporderid"]); ?>
							<span style="float: right"><?php echo ($pp_g["date"]); ?></span> 
						</span>
						<div class=" lout status_pp_p_<?php echo (get_pp_g_status($pp_g["pporderid"])); ?>" act="viewinfo" data="<?php echo ($pp_g["pporderid"]); ?>" style="margin-left:0px;">
							<span style="color: white;" class="orderid">订单号<?php echo ($pp_g["pporderid"]); ?></span>
							<span> (匹配卖出订单<?php echo ($pp_g['tgbz_data']['orderid']); ?>)</span>
							<div>
								<div class="bj status_pp_g_<?php echo (get_pp_g_status($pp_g["pporderid"])); ?>"></div>
								<div>
									<span class="bold"><?php echo ($pp_g['tgbz_user']['ue_theme']); ?> &gt; <span class="cash" style="color: white;"><?php echo ($pp_g["jb"]); ?></span> &gt; 您</span>
									<!--<span>订单创建日期:2019-05-17 10:32:24</span>-->
								</div>
							</div>
							<?php if($pp_g["zt"] == 0): ?><span style="color:#ff0000; position: inherit;" data="<?php echo (datedqsj($pp_g["date"])); ?>" class="dqsj countdownbox">剩余打款时间：</span><?php endif; ?>
							<?php if($pp_g["zt"] == 1): ?><span style="color:#ff0000; position: inherit;" data="<?php echo (dateqrdqsj($pp_g["date_hk"])); ?>" class="dqsj countdownbox">剩余确认时间：</span><?php endif; ?>
			...
			<div class="maincon">
				<div class="orderlist out">
					...
					<?php if(is_array($plist)): $i = 0; $__LIST__ = $plist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$p): $mod = ($i % 2 );++$i;?><a href="<?php echo U('orderdetail');?>?mainid=<?php echo ($p["mainid"]); ?>" style="text-decoration: none;">
						<div class="order_bar">
							...
							<p>开仓积分: <?php echo (get_kc_jb($p["mainid"])); ?>/<?php echo (get_p_status_yfkjb_new($p["mainid"],1)); ?>
							</p>
							<!--<p>平仓积分: <?php echo (get_pc_jb($p["mainid"])); ?>/<?php echo (get_p_status_yfkjb($p["orderid"],0)); ?></p>-->
							...
							<?php else: ?>
							<div class="status_p_<?php echo (get_p_status_pc($p["mainid"])); ?>"></div><?php endif; ?>
							<!---<div>等待平仓积分撮合交易</div>--->
						</div>
						</a>
						<!--<div style="background: #FFFFFF;" class="l_item sq out status_p_<?php echo (get_p_status($p["orderid"])); ?>" act="viewin" data="<?php echo ($p["orderid"]); ?>">
							<span class="sbj">开仓申请</span>
							<div>
								参与者:<?php echo ($p["user"]); ?>
							</div>
							<div>
								申请单编号:<?php echo ($p["orderid"]); ?>
							</div>
							<div>
