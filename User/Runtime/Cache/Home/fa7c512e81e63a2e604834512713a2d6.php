...
<div class="content-wrapper tab-content tab-addtabs" style=" padding-top: 0;">
		...
		<div class="maincon">
				...
				<?php if(is_array($pp_g_list)): foreach($pp_g_list as $key=>$pp_g): ?><div class="l_item lout status_pp_p_<?php echo (get_pp_g_status($pp_g["pporderid"])); ?>" act="viewinfo" data="<?php echo ($pp_g["pporderid"]); ?>" style="background: #002639; color: white;">
						...
						<div class=" lout status_pp_p_<?php echo (get_pp_g_status($pp_g["pporderid"])); ?>" act="viewinfo" data="<?php echo ($pp_g["pporderid"]); ?>" style="margin-left:0px;">
							...
							<div>
								...
								<div class="bj">
									<span class="cash" style="color: white;">需要收款金额：<?php echo ($pp_g["jb"]); ?>元 </span>
									<!--<span class="bold"><?php echo ($pp_g['tgbz_user']['ue_theme']); ?> &gt;-->
										<!--<span class="cash" style="color: white;"><?php echo ($pp_g["jb"]); ?></span> &gt;-->
										<!--您-->
									<!--</span>-->
							...
							<div>
								<div class="bj">
									<span>收款截至时间：</span><span><?php echo C('jjdktime_at');?></span>
								</div>
							</div>
							<?php if($pp_g["zt"] == 0): ?><span style="color:#ff0000; position: inherit;" data="<?php echo (datedqsj($pp_g["date"])); ?>" class="dqsj countdownbox">剩余收款时间：</span><?php endif; ?>
			...
			<div class="maincon">
				<div class="orderlist out">
					<!--<?php if($pcount == 0): ?><div style="color:#fff">当前没有交割订单</div><?php endif; ?>-->
					<?php if(is_array($jlist)): $i = 0; $__LIST__ = $jlist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$p): $mod = ($i % 2 );++$i;?><div class="order_bar">
							<h5><?php echo ($p["orderid"]); ?><span><?php echo ($p["date"]); ?></span></h5>
							<!--<p>持仓总量: <?php echo (get_jsbz_totaljb($p["mainid"])); ?>/<?php echo (get_pro_num(get_jsbz_totaljb($p["mainid"]))); ?> <noempty name="p.mainid">(<?php echo (get_pro_num(get_tgbz_totaljb($p["mainid"]))); ?>)<noempty></p>-->
							<p>交割积分: <?php echo (get_jsbz_totaljb($p["mainid"])); ?>/<?php echo (get_g_status_totaljb($p["orderid"])); ?></p>
							<!--<p>红利收益: <?php echo (get_tgbz_lx($p["id"])); ?></p>-->
							<div class="status_g_<?php echo (get_g_status($p["orderid"])); ?>"></div>
							<!--<div style="">撮合交易完成！请在2019-05-19 12:23:43之前交割，否则积分失效</div>-->
							<div class="order-list" style="display: none;">
								<?php if(is_array($p["ppdd_list"])): $i = 0; $__LIST__ = $p["ppdd_list"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$ppdds): $mod = ($i % 2 );++$i;?><ul class="order-list-a">
									<ul>
										<em class="em-left">订单号：<?php echo ($ppdds["pporderid"]); ?></em><br>
										<em>打款时间：<?php echo ($ppdds["date_hk"]); ?></em>
									</ul>
									<ul>
										<em class="em-left">积分:<?php echo ($ppdds["jb"]); ?></em><br>
										<em >状态：
											<?php if($ppdds["zt"] == 0): ?>已匹配<?php endif; ?>
											<?php if($ppdds["zt"] == 1): ?>已打款<?php endif; ?>
											<?php if($ppdds["zt"] == 2): ?>已完成<?php endif; ?>
										</em>
									</ul>
									<ul>
										<em>打款人：<?php echo ($ppdds["dk_ue_theme"]); ?></em><br>
										<em class="em-left">打款人账号：<?php echo ($ppdds["dk_ue_account"]); ?></em> <br>
										<em>支付宝:<?php echo ($ppdds["dk_zfb"]); ?></em> <br>
										<em class="em-left">微信:<?php echo ($ppdds["dk_weixin"]); ?></em> <br>
										<em>银行卡:<?php echo ($ppdds["dk_yhzh"]); ?></em>
									</ul>
	...
	<script>
        ...
        (function() {
			...
			$(document).on('click','.order_bar',function(){
				var is_block = $(this).children('.order-list').css('display');
				if( is_block == 'block' ){
                    $(this).children('.order-list').css('display','none');
				}else{
                    $(this).children('.order-list').css('display','block');
				}
			})
