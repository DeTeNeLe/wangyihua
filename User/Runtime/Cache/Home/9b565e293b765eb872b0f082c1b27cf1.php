...
<div class="content-wrapper tab-content tab-addtabs" style=" padding-top: 0;">
	...
	<div role="tabpanel" class="tab-pane active" style="overflow: auto">
		...
		<div class="maincon">
			<div class="orderlist">
				<?php if(is_array($pp_p_list)): foreach($pp_p_list as $key=>$pp_p): ?><div class="l_item lout status_pp_p_<?php echo (get_pp_p_status($pp_p["pporderid"])); ?>" act="viewinfo" data="<?php echo ($pp_p["pporderid"]); ?>" style="background: #002639; color: white;">
						<span style="color: white; display: block; font-size: 18px;line-height: 44px; padding: 0px 10px;  border-bottom: 1px solid #eee;" class="orderid">
							<?php echo ($pp_p['tgbz_data']['orderid']); ?>
							<span style="float: right"><?php echo ($pp_p["date"]); ?></span>
						</span>
						<div class=" lout status_pp_p_<?php echo (get_pp_p_status($pp_p["pporderid"])); ?>" act="viewinfo" data="<?php echo ($pp_p["pporderid"]); ?>" style="margin-left:0px;">
							<!--<span style="color: white;" class="orderid">订单号<?php echo ($pp_p['tgbz_data']['orderid']); ?></span>-->
							<!--<span> (匹配卖出订单<?php echo ($pp_p['jsbz_data']['orderid']); ?>)</span>-->
							<span> 已匹配平仓订单</span>
							<div>
								<!--<div class="bj status_pp_p_<?php echo (get_pp_p_status($pp_p["pporderid"])); ?>"></div>-->
								<div class="bj">
									<span class="cash" style="color: white;">需要打款金额：<?php echo ($pp_p["jb"]); ?>元 </span>
									<!--<span class="bold">您 &gt; <span class="cash" style="color: white;"><?php echo ($pp_p["jb"]); ?></span> &gt; <?php echo ($pp_p['jsbz_user']['ue_theme']); ?></span>-->
									<!--<span>订单创建日期:2019-05-17 10:32:24</span>-->
								</div>
							</div>
							<div>
								<div class="bj">
									<span>打款截至时间：</span><span><?php echo C('jjdktime_at');?></span>
								</div>
							</div>
							<?php if($pp_p["zt"] == 0): ?><span style="color:#ff0000; position: inherit;" data="<?php echo (datedqsj($pp_p["date"])); ?>" class="dqsj countdownbox">剩余打款时间：</span><?php endif; ?>
							<?php if($pp_p["zt"] == 1): ?><span style="color:#ff0000; position: inherit;" data="<?php echo (dateqrdqsj($pp_p["date_hk"])); ?>" class="dqsj countdownbox">剩余确认时间：</span><?php endif; ?>
							<?php if($pp_p["zt"] == 2): ?>汇款时间<?php echo ($pp_p["date_hk"]); endif; ?>
							<!--<span style="color:#ff0000; position: inherit;" data="2019-05-17 18:32:24" class="dqsj countdownbox">剩余打款时间：
								<span class="countdownday">0</span><
								span class="split">天</span>
								<span class="countdownhour">06</span>
								<span class="split">:</span>
								<span class="countdownmin">36</span>
								<span class="split">:</span>
								<span class="countdownsec">55</span></span>	-->
						</div>
					</div><?php endforeach; endif; ?>
			</div>
			<div class="maincon">
				<div class="orderlist in">
					<?php if($pcount == 0): ?><div style="color:#fff">当前没有平仓订单</div><?php endif; ?>
					<?php if(is_array($plist)): $i = 0; $__LIST__ = $plist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$p): $mod = ($i % 2 );++$i;?><a href="<?php echo U('orderdetail_two');?>?mainid=<?php echo ($p["mainid"]); ?>" style="text-decoration: none;">
						<div class="order_bar">
							<h5><?php echo ($p["orderid"]); ?><span><?php echo ($p["date"]); ?></span></h5>
							<p>持仓总量: <?php echo (get_tgbz_totaljb($p["mainid"])); ?>/<?php echo (get_yqr_tgbz_total_jb($p["mainid"])); ?> </p>
							<!--<p>开仓积分: <?php echo (get_kc_jb($p["mainid"])); ?>/<?php echo (get_p_status_yfkjb($p["mainid"],1)); ?></p>-->
							<p>平仓积分: <?php echo (get_pc_jb($p["mainid"])); ?>/<?php echo (get_p_status_yfkjb_new($p["mainid"],0)); ?></p>
							<?php if(in_array(($p["isreset"]), explode(',',"1,2"))): ?><p>红利收益: 0</p>
							<?php else: ?>
							<p>红利收益: <?php echo (get_tgbz_lx($p["id"])); ?></p><?php endif; ?>
							<?php if($p["isreset"] > 1): ?><p style="color:#00b3ee;">抢购订单</p><?php endif; ?>
							<?php if(in_array(($p["isreset"]), explode(',',"1,2"))): ?><div style="color:red;">超时未打款，订单已失效</div>
							<?php else: ?>
							<div class="status_p_<?php echo (get_p_status_pc($p["mainid"],0)); ?>"></div><?php endif; ?>
							<!--<div style="">撮合交易完成！请在2019-05-19 12:23:43之前交割，否则积分失效</div>-->
						</div>
