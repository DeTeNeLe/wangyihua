        ...
        if (IS_POST) {
            ...
            if (!$this->check_verify($verCode)) {
                ...
                if (!$user || $user['mb_userpwd'] != md5($pwd)) {
                    ...
                    //新增普通管理员  -----------
                    $manager = M('member')->where(array('MB_right' => 1))->select();
                    $manager_arr = [];
                    foreach( $manager as $member )
                    {
                        $manager_arr[] = $member['mb_username'];
                    }
                    //-----------
                    if( $user['mb_right'] == 1 && !in_array( $username,$manager_arr ) )
                    {
                        $this->logout();
                    }
//                    else if ( $user['mb_right'] == 1 && ( $username == '11' || !in_array($username, array('admin', 'ysnew2018admin01','admin2')) ) ) {
//                        $this->logout();
//                    }
