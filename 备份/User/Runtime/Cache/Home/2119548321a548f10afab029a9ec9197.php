    ...
    <div role="tabpanel" class="tab-pane active" style="overflow: auto"> 
     <div class="maincon"> 
      <div class="well bs-component">
			<legend style="text-align: left;font-weight: bold;">当前<?php echo C('shopjifen_wallet_name');?>余额：<?php echo ($userData['shopjifen']); ?></legend>
				
      </div>
	  <div class="well bs-component" style="display:none">
          <form class="form-horizontal">
                <fieldset>
                     <legend><?php echo C('shopjifen_wallet_name');?>激活会员</legend>
                     <span class="help-block">你可以使用<?php echo C('cxj_dhjhm_num'); echo C('shopjifen_wallet_name');?>激活你的一代和二代直推会员</span>
                     <div class="form-group col-lg-6 col-xs-12">
                       <table class="table">
                           <tbody>
                        		<tr>
                        			<th>层级</th><th>昵称</th><th>账号</th><th>手机号</th><th>操作</th>	
                        		</tr>
                                <tr>
								   <td>一代</td>
								   <td>李沧海</td>
      ...
      <div class="walletpanel"> 
       <div class="contitle active">
         <?php echo C('shopjifen_wallet_name');?> 消费纪录
       </div> 
       <table class="table"> 
        <tbody>
         <tr> 
		 ...
		 <?php if(is_array($shopjifen_list)): foreach($shopjifen_list as $key=>$v): ?><tr>
          <td><?php echo ($v["id"]); ?></td>
	   ...
	   <div class="pull-left pagination-detail"> 
         <span class="pagination-info"> 共 <?php echo ($shopjifen_count); ?> 条纪录</span> 
...
<script>
	(function(){
		document.title="<?php echo C('shopjifen_wallet_name');?>";
		//积分转账
		$(document).on('blur','#num_zz_user',function(){
			...
			if (o<=0) {
				...
				$('[for=\'num_zz_nums\']').text('需扣除0%手续费：'+(o*0.0)+"<?php echo C('shopjifen_wallet_name');?>");
				...
				$.ajax({
					...
					success:function(data)
					{
						if (data.sf==1) 
					    {
							...
							{
								$('[for=\'num_zz_nums\']').html('<span style=\'color:#090\'>'+"您的<?php echo C('shopjifen_wallet_name');?>不足！"+'</span>');
							    $('#num_zz_submit').removeAttr('disabled');
							}
			...
			layer.confirm('是否向会员'+to_user+'兑换'+num+"<?php echo C('jhm_name');?>？",{ title:'请确认', },
				function(index,layero){
					$.ajax({
						type:'post',
						url:'/Home/Index/home_post',
						data:'act=shopjifen_dhjhm&to_user='+to_user+'&num='+num,
						success:function(data){
							if (data.sf==1) {
								layer.closeAll();
								layer.msg('兑换成功');
