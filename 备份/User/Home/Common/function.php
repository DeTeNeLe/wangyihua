...
function fftuijianmoney($var,$money, $level,$tgbzid){    //$level --------->代数
	...
	file_put_contents("tuijian.txt","########" . $var . "," . $level . "," . $levelname  . "," . $accname_zq['ue_accname'], FILE_APPEND);
...
function tuijianAccountmoney($account,$money,$levelnum ){
   ...
   //echo  M('user')->where(array('UE_account' => $account))->getlastSql()."</br>";
	...
	$record3 ["UG_account"] = $account; // 登入轉出賬戶
	$record3 ["UG_type"] = 'jb';
	$record3 ["UG_allGet"] = $accname_zq['qwe']; // 金幣
	...
	if(C("tx_jifen") > 0 )
	{
		...
		$record2 ["UG_type"] = 'jb';
		$record2 ["UG_allGet"] = $accname_zq['shopjifen']; // 金幣
 ...
 * 自动封号
 * @param $ppid 匹配订单号码
 * @param $username 提供帮助的用户名
 */

function ban_users($ppid)
{
    $time = NOW_TIME;
    $ppmx =M('ppdd')->where(array('id'=>$ppid,'zt'=>0))->find();
    if(!$ppmx)
	{
        return;
    }

	$tg_user = M('user')->where(array('UE_account'=>$ppmx['p_user']))->find(); 
    $up_user = M('user')->where(array('UE_account'=>$tg_user['ue_accname']))->find();


    //超时不确认扣除自己的积分
	if($ppmx['date_hk'] != "" && $ppmx['date_hk'] !=null  && ($time - strtotime($ppmx['date_hk']))/3600 >= C('jjqrtime'))
	{
		M('user')->where(array('UE_ID'=>$up_user['ue_id']))->setDec('jifen',C('chaoshi_kcjf'));
    ...
    //超时不打款扣除上级奖金,永久封号
    if(($time - strtotime($ppmx['date']))/3600 >= C('jjdktime'))
	{
		M('user')->where(array('UE_ID'=>$tg_user['ue_id']))->setField('UE_status',1);

		if(C('jjhydjkcsjmoeney') > 0)
		{
        ...
        M('jsbz')->where(array('id'=>$ppmx['g_id']))->setField('zt',0);
		M('jsbz')->where(array('id'=>$ppmx['g_id']))->setField('drwsk',1);
        M('jsbz')->where(array('id'=>$ppmx['g_id']))->setField('jb',$tg['jb']);
        M('tgbz')->where(array('id'=>$ppmx['p_id']))->delete();
        M('ppdd')->where(array('id'=>$ppid))->delete();
    }
...
{
    $list = M('ppdd')->where(array('zt'=>0))->select();
    if(!$list){
        return;
    }
    foreach($list as$k=>$i){
...
function ppdd_add($p_id,$g_id){
   ...
   // echo $p_user1['user'].'<br>';
	if(M('ppdd')->add($data_add)){
		...
		$get_user=M('user')->where(array('UE_account'=>$p_user1['user']))->find();
		if($get_user['ue_phone'] && $tdcount==1 && C('sms_open_pp')=="1"){ 
			sendSMS($get_user['ue_phone'],"亲爱的会员您好，您申请提供的订单已匹配，及时处理【" . C('sms_sign') . "】");
			insetSMSLog($get_user['ue_account'],$get_user['ue_phone'],3,"亲爱的会员您好，您申请提供的订单已匹配，及时处理【" . C('sms_sign') . "】");
		}
		return true;	
	}else{
		return false;
