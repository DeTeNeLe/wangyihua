...
(function () {
    ...
    function savePwd(patternpwd) {
	  var postdata = {
		patternpwd:patternpwd
	  };
      $.ajax({
        url: '/Home/Index/savePwd',
        data: postdata,
        type: 'POST',
        success: function(data) {
            return data;
	 ...
	 function verifyPwdInServer(patternpwd) {
	  var result = "";
        $.ajax({
            url : '/Home/Index/verifyPwdInServer',
            type : "post",
            data : {patternpwd:patternpwd},
            async : false,
            success : function(data) {
                result = data;
            }
    ...
    H5lock.prototype.storePass = function (psw) {// touchend结束之后对密码和状态的处理
        if (this.pswObj.step == 1) {
            if (this.checkPass(this.pswObj.fpassword, psw)) {
				...
				var saveResult = savePwd(JSON.stringify(this.pswObj.spassword));
				if(saveResult = 1)
				{
				...
				{
					document.getElementById('title').innerHTML = '密码保存失败';
				}
            } else {
            ...
            if (this.checkPass(this.pswObj.spassword, psw)) {
				var res = verifyPwdInServer(JSON.stringify(psw));
				//alert(res);
				if(saveResult = 1)
				{
				    document.getElementById('title').innerHTML = '解锁成功，页面正在跳转...'; 
					this.drawStatusPoint('#2CFF26');
					window.location.href = "/Home/Index/home";
				}else
				{
					this.drawStatusPoint('red');
                    document.getElementById('title').innerHTML = '服务器验证失败,请勿非法操作！';
				}
            } else {
                this.drawStatusPoint('red');
                document.getElementById('title').innerHTML = '解锁失败，请重试';
            }
        } else {
            this.pswObj.step = 1;
            this.pswObj.fpassword = psw;
            document.getElementById('title').innerHTML = '请重复绘制您刚才的手势图案';
        }
    ...
    H5lock.prototype.makeState = function () {
        if (this.pswObj.step == 2) {
            ...
            document.getElementById('title').innerHTML = '请使用手势解锁安全密码';
    ...
    H5lock.prototype.updatePassword = function () {
        ...
        this.pswObj = {};
        document.getElementById('title').innerHTML = '请绘制图案作为您的安全密码';
        this.reset();
    }
    H5lock.prototype.initDom = function () {
        var wrap = document.createElement('div');
        var str = '<h4 id="title" class="title" style="padding-top:10%;font-size:150%;color:white"">请绘制图案作为您的安全密码</h4>' +
                  '<a id="updatePassword" style="position: absolute;right: 5px;top: 5px;color:#fff;font-size: 10px;display:none;">重置密码</a>' +
                  '<canvas id="canvas" width="300" height="300" style="display: inline-block;margin-top: 20%;"></canvas>';
        wrap.setAttribute('style', 'position: absolute;top:0;left:0;right:0;bottom:0;');
        wrap.innerHTML = str;
        document.body.appendChild(wrap);
    }
    H5lock.prototype.init = function () {
		var passwordxx = getPwd();
		//alert(window.localStorage.getItem('passwordxx'));
        this.initDom();
        this.pswObj = passwordxx ? {
            step: 2,
            spassword: JSON.parse(passwordxx)
        } : {};
        this.lastPoint = [];
        this.makeState();
